...
Mesh::Mesh(string filename, int inv) {
...
void Mesh::load(string filename, int inv) {
	// Release resources
	release();
	...
	while (getline(file, line)) {
		if (line.substr(0, 2) == "v ") {
			...
			vec3 vert(stof(values[0])+addx+1, inv* stof(values[1])+1, stof(values[2])+1);
	...
	for (int i = 0; i < raw_vertices.size(); i++)
		global_vertices.push_back(raw_vertices[i]);
	// Create vertex array
